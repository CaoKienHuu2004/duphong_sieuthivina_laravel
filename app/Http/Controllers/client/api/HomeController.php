...
{
    return QuatangsukienModel::with('sanphamduoctang.sanpham.thuonghieu')
        ...
        ->get()
        ->map(function ($event) {
            // Kiểm tra xem sự kiện có sản phẩm tặng không để tránh lỗi
            if ($event->sanphamduoctang) {
                
                // VÌ MỘT SỰ KIỆN CÓ NHIỀU SẢN PHẨM -> PHẢI DÙNG VÒNG LẶP FOREACH
                foreach ($event->sanphamduoctang as $item) {
                    
                    // Kiểm tra null an toàn (tránh lỗi nếu sản phẩm bị xóa)
                    if ($item->sanpham && $item->sanpham->thuonghieu) {
                        
                        $currentLogo = $item->sanpham->thuonghieu->logo;

                        // Chỉ gán asset nếu logo chưa có http
                        if (!empty($currentLogo) && !str_starts_with($currentLogo, 'http')) {
                            $item->sanpham->thuonghieu->logo = asset('assets/client/images/brands/' . $currentLogo);
                        }
                    }
                }
            }
