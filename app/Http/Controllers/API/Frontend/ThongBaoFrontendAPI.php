...
class ThongBaoFrontendAPI extends BaseFrontendController
{
     ...
     *     description="Tạo thông báo mới cho người dùng đang đăng nhập",
     *     security={{"bearerAuth":{}}},
     *     @OA\RequestBody(
     *         required=true,
     *         @OA\JsonContent(
     *             required={"tieude","noidung"},
     *             @OA\Property(property="tieude", type="string", example="Thông báo mới"),
     *             @OA\Property(property="noidung", type="string", example="Nội dung thông báo"),
     *             @OA\Property(property="lienket", type="string", example="https://example.com"),
     *             @OA\Property(property="trangthai", type="string", enum={"Đã đọc","Chưa đọc","Tạm ẩn"}, example="Chưa đọc")
     *         )
     *     ),
     *     @OA\Response(
     *         response=201,
    ...
    public function store(Request $request)
    {
        ...
        $validated = $request->validate([
            ...
            'trangthai' => 'nullable|in:Đã đọc,Chưa đọc,Tạm ẩn',
        ]);

        $thongbao = ThongbaoModel::create(array_merge($validated, [
            'id_nguoidung' => $user->id,
            'trangthai' => $validated['trangthai'] ?? 'Chưa đọc',
        ]));
     ...
     *     description="Cập nhật thông tin thông báo của người dùng",
     *     security={{"bearerAuth":{}}},
     *     @OA\Parameter(
     *         name="id",
     *         in="path",
     ...
     *         @OA\JsonContent(
     *             @OA\Property(property="tieude", type="string", example="Thông báo mới"),
     *             @OA\Property(property="noidung", type="string", example="Nội dung mới"),
     *             @OA\Property(property="lienket", type="string", example="https://example.com"),
     *             @OA\Property(property="trangthai", type="string", enum={"Đã đọc","Chưa đọc","Tạm ẩn"}, example="Đã đọc")
     *         )
     *     ),
     *     @OA\Response(
     *         response=200,
